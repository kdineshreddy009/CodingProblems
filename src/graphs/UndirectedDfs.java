...
/*
https://practice.geeksforgeeks.org/problems/depth-first-traversal-for-a-graph/1/?track=SPC-Graph&batchId=145
...
public class UndirectedDfs {
	...
	private static void findDfs(ArrayList<ArrayList<Integer>> adjacencyList, int vertex) {
		result.add(vertex);
		visited[vertex] = 1;
		for (int i = 0; i < adjacencyList.get(vertex).size(); i++) {
            // 
			int new_vert = adjacencyList.get(vertex).get(i);
			if (visited[new_vert] != 1) {
				findDfs(adjacencyList, new_vert);
			}
		}
	}
}